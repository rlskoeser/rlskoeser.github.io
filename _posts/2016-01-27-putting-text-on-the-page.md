...
One of the projects I've had the privilege to working on over the last
...
(more on the [project history](http://readux.library.emory.edu/about/)).
...
BookReader](https://github.com/openlibrary/bookreader), or the IIIF
viewer [Mirador](http://projectmirador.org/)), but it inevitably it
seems they are written in a different language, or difficult to adapt,
modify, and integrate into a larger application and.   The technologies
underlying these applications are all probably pretty similar: page
images, scans or photographs of physical book pages, with text somewhere
that has been generated through OCR (optical character recognition),
usually used for search but not display, since OCR tends to be
unreliable.  Some of those interfaces (like the Internet Archive
...
and then it supplies the text from that section of the page (which is
fairly neat, but not always as useful as you might want, because you can
only select images in rectangular shapes, which doesn't always match the
text flow).
...
cases, the OCR is really quite terrible and unreadable).  But by putting
the text on the page, users can take advantage of all the normal text
...
converted our OCR XML to [TEI facsimile](http://www.tei-c.org/release/do
...
Sometimes it's a lot easier to look at examples, so here are some links
for content in Readux and some of the code I mentioned.  If you're
interested in the HTML layout and positioning, use your browser
developer tools on some Readux volume pages to inspect some of the text
content.
